...
Here is where things got to be a little sticky for us. In our first attempt (per the code below), we were only able to find 12 of the 13 owners mentioned by 538. We filtered for election year 2020, used string detect "Trump" to find committees in the recipient column supporting Trump's reelection. We saved these 12 owners in the dataframe "trump_2020" and we know that their contributions total to $1,544,421. That leaves one owner and $202,002 in donations missing. 
...
Examening this list, many donations appear to be for candidates for Senate. One PAC stood out to us as appearing to be for the presidential race and specifically for Trump: "BLACK AMERICANS TO RE-ELECT THE PRESIDENT" in our list of committees we were filtering by. Let's add that PAC to our code and see what the new total is. 
...
Okay, we are still missing a big chunk of money! What PAC are we not seeing that supports Trump? We wondered if 538 was including certain national, party-based committees recipients in their total for Trump, such as WINRED and the REPUBLICAN NATIONAL COMMITTEE. However, including either or both of those committees still yielded the incorrect total.
...
Now we were taking on a data analysis method we're not proud of: scanning non-Democratic 2020 donations by our 13 owners and seeing if they were in this list. 

Bill Foley (Vegas Golden Knights), Bill Hornbuckle (Las Vegas Aces), Charles Johnson (San Francisco Giants), Dan Devos (Orlando Magic), Jack Roush (NASCAR), James Dolan (New York Knicks and Rangers), Kelly Loeffler (Atlanta Dream), Mike Brown (Cincinnati Bengals), Peter Karmanos (Carolina Hurricanes), Robert Plummer (Los Angeles Dodgers), Roger Penske (Team Penske) Tilman Fertitta (Houston Rockets) and Todd Ricketts
```{r}
...
Finally we noticed that America First Action Inc. received $200,000 in 2020 from Dan Devos, who was in our list of the 13 owners. This was the exact amount to make up the remaining gap between our total and 538's total, and according to the Center for Public Integrity this PAC supported Trump in 2020. 

Below is the code that enabled us to successfully replicate 538's finding at last. 
...
On to the second part of Finding 5: "By contrast, 20 owners have contributed directly to Biden or Biden’s super PACs, though at smaller totals, with overall contributions equaling $405,745."

Like above, let's start by filtering by 2020 and recipients that include "Biden" to see how many owners we have. 
...
# We have 20 rows! Amazing. Now how much have they collectively donated to Biden? 
...
```
We knew we were yet again missing some PACs that didn't have "Biden" in the name. So again we looked back at the 538 article for the list of owners and found: 
...
Why isn't 538 including Adam Silver? Let's find him in the original donation_df dataframe. Aha! He does not have a team listed - he is a commissioner! That must be why 538 didn't count him in the list of 20 team owners who supported Biden. This made us scratch our heads at why the commissioners would be included in the dataframe just to be excluded a stat like this. But that's a question for another day. 
...
Okay, how did we also miss John Rogers? Let's look him up too. 
...
Ah, we missed him because none of the PACs he donated to included "Biden." We started Googling these PACs to figure out which Rogers donation was actually supporting Biden. Again, this was time consuming and not at all efficient. We were left pining for a column that indicated whether a PAC was for congress or the presidential election. At long last, we found that "UNITE THE COUNTY" supported Biden in 2020. https://unitethecountry.com/

```{r}
# Let's revisit the biden_2020 dataframe, adding code to exclude commissioners and include the UNITE THE COUNTRY PAC. 
...
There is still at least one missing pack and $100,000 in donations to Biden unaccounted for. This was not pretty, but let's look at the PACs that all 20 confirmed Biden-supporting owners gave to in 2020. And let's keep Googling them. 
